...
public class PaymentContorller {
    ...
    @PostMapping(value = "consul/payment/get/{id}")
    public CommonResult get(@PathVariable("id") int id){
        ...
        log.info(payment.toString());
        if (payment != null) {
            return new CommonResult(1,"查询成功,"+port+"端口",payment);
        }else {
             return new CommonResult(0,"此ID不存在");
        }
    }
   @PostMapping(value = "consul/payment/add")
    ...
    @RequestMapping(value = "consul/payment/client")
    public CommonResult cliceString() {
        ...
        if (js !=null) {
            return new CommonResult(1,"查询成功，"+port+"端口",js);
        }else{
